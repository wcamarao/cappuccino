...
module.exports.any = matcher.any;
...
function when(mock) {
  ...
  function matchers(method, arguments) {
    for (index in arguments) {
      var argument = arguments[index];
      state.matcher(mock, method, index, matcher.identify(argument));
    }
  }
...
function unexpectedArgument(method, expectation, argument, index) {
  var title = method + '(), argument #'+(parseInt(index)+1);
  var description = 'expected '+expectation+', but actually received '+argument;
